...
import { ErrorsService, NotificationsService, ReappointmentsService, StatusCategoriesService } from '@app/shared/services';
import { DynamicDialogConfig, DynamicDialogRef, SelectItem, SelectItemGroup } from 'primeng';
import { Subscription } from 'rxjs';

@Component({
  selector: 'app-reappointment-change-status-form',
  templateUrl: './reappointment-change-status-form.component.html',
  styleUrls: ['./reappointment-change-status-form.component.css']
})
export class ReappointmentChangeStatusFormComponent implements OnInit, OnDestroy {
  ...
  saveData() {
    if (this.entityId) {
      ...
      this.subscription.add(this.reappointmentsService.updateStatus(this.entityId, formStatusCategoryId).subscribe(res => {
        this.ref.close();
