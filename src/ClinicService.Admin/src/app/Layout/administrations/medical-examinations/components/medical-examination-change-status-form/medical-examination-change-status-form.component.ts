...
import { ErrorsService, MedicalExaminationsService, NotificationsService, StatusCategoriesService } from '@app/shared/services';
import { DynamicDialogConfig, DynamicDialogRef, SelectItem, SelectItemGroup } from 'primeng';
import { Subscription } from 'rxjs';

@Component({
  selector: 'app-medical-examination-change-status-form',
  templateUrl: './medical-examination-change-status-form.component.html',
  styleUrls: ['./medical-examination-change-status-form.component.css']
})
export class MedicalExaminationChangeStatusFormComponent implements OnInit, OnDestroy {
  ...
  saveData() {
    if (this.entityId) {
      ...
      this.subscription.add(this.medicalExaminationsService.updateStatus(this.entityId, formStatusCategoryId).subscribe(res => {
        this.ref.close();
